...
<?php $__env->startPush('meta'); ?>
    ...
    <?php if(core()->getConfigData('catalog.rich_snippets.categories.enable')): ?>
        <script type="application/ld+json">
            <?php echo app('Frooxi\Product\Helpers\SEO')->getCategoryJsonLd($category); ?>
...
<?php $component->withAttributes([]); ?>
            ...
            <div style="padding-bottom:8px;">
                ...
                <div id="pc-color-body" style="padding:4px 28px 20px;display:grid;grid-template-columns:1fr 1fr;gap:4px 12px;">
                    ...
                    <?php $__currentLoopData = $colorList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $col): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div
                            id="pc-color-row-<?php echo e($col['id']); ?>"
                            onclick="pcToggleColor(this,'<?php echo e($col['id']); ?>')"
                        ...
                        >
                            <div id="pc-cb-color-<?php echo e($col['id']); ?>" style="width:16px;height:16px;border-radius:4px;border:1.5px solid #d1d5db;flex-shrink:0;display:flex;align-items:center;justify-content:center;transition:all .15s;"></div>
    ...
    <?php if (! $__env->hasRenderedOnce('7688313e-619e-461f-ba55-ec8190450a15')): $__env->markAsRenderedOnce('7688313e-619e-461f-ba55-ec8190450a15');
$__env->startPush('scripts'); ?>
        <script>
            (function () {
                var CATEGORY_ID = <?php echo json_encode((string) $category->id, 15, 512) ?>;
                var PRODUCTS_API_URL = <?php echo json_encode(route('shop.api.products.index'), 15, 512) ?>;
                var WISHLIST_INDEX_URL = <?php echo json_encode(route('shop.api.customers.account.wishlist.index'), 15, 512) ?>;
                var WISHLIST_STORE_URL = <?php echo json_encode(route('shop.api.customers.account.wishlist.store'), 15, 512) ?>;
                ...
                var LOGIN_URL = <?php echo json_encode(route('shop.customer.session.index'), 15, 512) ?>;
                var PRICE_RANGE_URL = <?php echo json_encode(route('shop.api.categories.price_range', ['id' => $category->id]), 512) ?>;
                var EMPTY_TEXT = <?php echo json_encode(trans('shop::app.categories.view.empty'), 15, 512) ?>;
                var LOAD_MORE_TEXT = <?php echo json_encode(trans('shop::app.categories.view.load-more'), 15, 512) ?>;
                var ADD_TO_WISHLIST_LABEL = <?php echo json_encode(trans('shop::app.components.products.card.add-to-wishlist'), 15, 512) ?>;
                var isCustomer = <?php echo json_encode(auth()->guard('customer')->check(), 15, 512) ?>;
                var wishlistEnabled = <?php echo json_encode((bool) core()->getConfigData('customer.settings.wishlist.wishlist_option'), 15, 512) ?>;
...
<?php endif; ?><?php /**PATH /Users/tasfirulhaque/Frooxi/Iqbal Hossen/bagisto_ecom/packages/Frooxi/Shop/src/resources/views/categories/view.blade.php ENDPATH**/ ?>