...
// REMOVED: Marketing package on hold
// use Frooxi\Marketing\Repositories\SearchSynonymRepository;
...
class ProductRepository extends Repository
{
    ...
    public function searchFromDatabase(array $params = [])
    {
        ...
        $query = $this->with([
            ...
            if (! empty($params['category_id'])) {
                ...
                $qb->whereIn('product_categories.category_id', array_unique($allCategoryIds));
            }
            ...
            foreach ($attributes as $attribute) {
                ...
                if ($attribute->code == 'name') {
                    // REMOVED: Marketing package on hold - synonyms disabled
                    // $synonyms = $this->searchSynonymRepository->getSynonymsByQuery(urldecode($params['name']));
                    $synonyms = [urldecode($params['name'])];
    ...
    {
        if ($this->searchEngine == 'elastic') {
            return $this->elasticSearchRepository->getMinPrice($params);
        }
        ...
        if (! empty($params['category_id'])) {
            $categoryIds = is_array($params['category_id']) ? $params['category_id'] : explode(',', $params['category_id']);
            $allCategoryIds = [];
            ...
            $query->whereIn('product_categories.category_id', array_unique($allCategoryIds));
        }
     ...
     * @return array
     */
    protected function getChildCategoryIds($categoryId)
    {
